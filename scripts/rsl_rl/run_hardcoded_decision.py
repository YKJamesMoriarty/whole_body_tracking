...
parser.add_argument(
    ...
    type=str,
    nargs="+",
    default=["cross", "swing", "roundhouse"],
    help="技能循环顺序",
)
parser.add_argument(
    ...
    default=True,
    help="是否对 roundhouse 使用固定目标点列表采样（用于 demo 稳定性）",
)
parser.add_argument(
...
parser.add_argument("--model_dir", type=str, default="basic_model", help=".pt 模型目录")
parser.add_argument("--motion_dir", type=str, default="Reference_Motion_IROS", help=".npz 动作目录")
...
def main(env_cfg: ManagerBasedRLEnvCfg, agent_cfg):  # type: ignore[override]
    ...
    for trial_idx in range(num_trials):
        ...
        if attack_skill == "roundhouse" and args_cli.use_roundhouse_fixed_points:
