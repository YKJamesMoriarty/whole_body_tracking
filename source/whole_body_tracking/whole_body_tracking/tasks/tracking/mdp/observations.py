...
def target_relative_position(env: ManagerBasedEnv, command_name: str, effector_body_name: str = "left_wrist_yaw_link") -> torch.Tensor:
    """
    (1) 目标相对位置 (3 维): [x, y, z]
...
def skill_type_one_hot(env: ManagerBasedEnv, command_name: str) -> torch.Tensor:
    ...
    one_hot[:, 7] = 1.0  # 例如 one_hot[:, 0] = 1.0 为 直拳 (Stage 1)
