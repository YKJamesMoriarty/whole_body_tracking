# DeepStrike: A Hierarchical Framework for Context-Aware Skill Selection and High-Fidelity Humanoid Striking
...
|--------|--------|----------|-------------|-------------------|-----------------|
| `basic_attack_right_cross` | Cross (右直拳) | 右手腕 | `cross_right_normal_body.npz` | `right_wrist_yaw_link` | 0 |
| `basic_attack_right_swing` | Swing (右摆拳) | 右手腕 | `cross_right_normal_body.npz` | `right_wrist_yaw_link` | 1 |
| `basic_attack_right_roundhouse_low` | Roundhouse Low (右摆拳) | 右脚踝 | `roundhouse_right_normal_low.npz` | `right_ankle_roll_link` | 2 |
| `basic_attack_right_roundhouse_high` | Roundhouse High (右高位鞭腿) | 右脚踝 | `roundhouse_right_fast_high.npz` | `right_ankle_roll_link` | 3 |
| `basic_attack_right_frontkick` | Front Kick (右脚正蹬) | 右脚踝 | `frontkick_right_normal_body.npz` | `right_ankle_roll_link` | 4 |

关键调试参数:  
- 攻击肢体: 右脚踝 (`right_ankle_roll_link`)、右手腕 (`right_wrist_yaw_link`)、左脚踝(`left_ankle_roll_link`)、左手腕(`left_wrist_yaw_link`)
- 目标高度: Frontkick, z = -0.3; Cross, z = 0.4; Roundhouse_high, z = 0.05; Swing, z = 0.6;
(相对于参考动作第一帧 Pelvis 局部坐标系)
  - x=0.5m (前方50cm), y=0 (居中), z=-0.3m (骨盆以下30cm)
- skill_type one-hot: 如索引4 对应 `frontkick_right_normal_body`
...
### 数据来源
- 格斗动作: HKUST(GZ)-HCL实验室通过运动捕获设备录制并重定向到 G1 机器人
...
如有问题可以联系作者: kyang556@connect.hkust-gz.edu.cn
